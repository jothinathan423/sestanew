...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $target_dir = "caprofile/";
    $target_file = $target_dir . basename($_FILES["profile_picture"]["name"]);

    $check_query = "SELECT * FROM ca WHERE ca_id='$id' OR username='$email'";
    $check_result = $conn->query($check_query);

    if ($check_result->num_rows > 0) {
        // CA with the same ID or email already exists
        echo '<script>alert("CA with the same ID or email already exists!"); window.location="caregister.php";</script>';
        exit();
    }
    ...
    if (move_uploaded_file($_FILES["profile_picture"]["tmp_name"], $target_file)) {
        ...
        $sql = "INSERT INTO ca (ca_id, name, password, username, phonenumber, gender, department, images)
