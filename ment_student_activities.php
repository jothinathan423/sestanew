...
if (isset($_POST['action'])) {
    ...
    if ($action === 'accept') {
        ...
        $creditPointsQuery = "SELECT credit_points FROM student_activities WHERE id = $activity_id";
        ...
        if ($creditPointsResult->num_rows > 0) {
            ...
            $updateStudentCreditPointsQuery = "UPDATE students SET credit_points = credit_points + $creditPoints WHERE id = $student_id";
            if ($conn->query($updateStudentCreditPointsQuery) === TRUE) {
                // Update the status of the activity
                $updateActivityStatusQuery = "UPDATE student_activities SET status = 'approved' WHERE id = $activity_id";
        ...
        $activity_id = $_POST['activity_id'];
        $deleteQuery = "DELETE FROM student_activities WHERE id = $activity_id";
...
if ($result_data->num_rows > 0) {
    while ($row = $result_data->fetch_assoc()) {
        ...
        echo "<div class='row'>
            <div class='col-md-6'><h5 class='card-text'>Total Credit Point</h5></div>
            <div class='col-md-6'>:  300</div>
        </div>";
        echo "<div class='row'>
            <div class='col-md-6'><h5 class='card-text'>Earned Credit Point</h5></div>
            ...
            <div class="row">
              <div class="col-md-12 grid-margin stretch-card">
                ...
                <div class="card-body" id="students">
                  <h2 class='mt-4'>SESTA Activities List:</h2>
...
while ($row = $result->fetch_assoc()) {
    ...
    if ($row['status'] !== 'approved') {
        ...
        echo "<input type='hidden' name='activity_id' value='" . $row['id'] . "'>";
        echo "<input type='hidden' name='student_id' value='" . $student_id . "'>";
        echo "<button type='submit' name='action' value='accepta' class='btn btn-success'>Accept</button>";
        echo "</form>";
        echo "<form method='post'>";
        echo "<input type='hidden' name='activity_id' value='" . $row['id'] . "'>";
        echo "<input type='hidden' name='student_id' value='" . $student_id . "'>";
        echo "<button type='submit' name='action' value='deletea' class='btn btn-danger'>Delete</button>";
        echo "</form>";
    }
