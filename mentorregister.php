...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $target_dir = "mentorprofile/";
    $target_file = $target_dir . basename($_FILES["profile_picture"]["name"]);

    $check_query = "SELECT * FROM mentors WHERE mentor_id='$id' OR email='$email'";
    $check_result = $conn->query($check_query);

    if ($check_result->num_rows > 0) {
        // CA with the same ID or email already exists
        echo '<script>alert("Mentor with the same ID or email already exists!"); window.location="mentorregister.php";</script>';
        exit();
    }
    ...
    if (move_uploaded_file($_FILES["profile_picture"]["tmp_name"], $target_file)) {
        ...
        $sql = "INSERT INTO mentors (mentor_id, mentor_name, password, email, phonenumber, gender, department, images)
...
?>
    <div class="container">
        <header>Mentor Registration</header>
